<?php
        ...
        if($login=='yes')
    {
        ...
        if(mysql_query($data))
        {
            ...
            echo"<script>location='willysfruit.php?yon=yes&id=".$prodid."&login=$login&uid=$uid&fname=$fname&lname=$lname'</script>";
        }
        else
        {
            echo"<script>alert('Product already placed in your Cart!');</script>";
            echo"<script>location='willysfruit.php?yon=no&id=".$prodid."&login=$login&uid=$uid&fname=$fname&lname=$lname'</script>";
        }
    }
    else
    {
